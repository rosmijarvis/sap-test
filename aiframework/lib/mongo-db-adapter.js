...
class MongoDBAdapter{
	...
	fetchBotSessionID(userId, configurationCollection, callbackFunction)
	{
		
		console.log('Checking the session for id  ::  ' +userId);
		MongoClient.connect(url, function(err, db) {
			assert.equal(null, err);
			//Fetch the bot configuration
			db.collection(configurationCollection).find({'_id': userId}).toArray(function (err, result) {
				...
				{	
					console.log("The sessionId for user "+userId+" is "+result);
	...
	fetchBotUserID(userId, configurationCollection, callbackFunction)
	{
		console.log('Checking if the user id exists ::  ' +userId);
		MongoClient.connect(url, function(err, db) {
			assert.equal(null, err);
			//Fetch the bot configuration
			db.collection(configurationCollection).find({'_id': userId}).toArray(function (err, result) {
				...
				{	
			        console.log('Document found with defined "find" criteria!');
					callbackFunction(true);
				}
				else 
				{
	...
	// find the configuration for a Bot
	fetchLastCommunication(botName, configurationCollection, callbackFunction)
	{
		var configuration='';
		console.log('Fetching the configuration for Bot : ' + botName);
		MongoClient.connect(url, function(err, db) {
			assert.equal(null, err);
			//Fetch the bot configuration
			db.collection(configurationCollection).find({'_id': botName}).toArray(function (err, result) {
				if (err) 
				{
					console.log(err);
				}
				...
				{				
					...
					{
						configuration = result[0];
						console.log(configuration.lastTransactionDatetime);
						// Invoke the passed callback function with the configuration as a parameter
						//callbackFunction(configuration);
					}
				}
				...
				callbackFunction(configuration.lastTransactionDatetime);
				//Close connection
				db.close();
			});
		});		
